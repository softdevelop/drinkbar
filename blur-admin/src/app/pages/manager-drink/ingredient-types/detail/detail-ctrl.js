...
(function () {
	...
	function IngredientTypesDetailCtrl($stateParams, $scope, IngredientTypesService, toastr, $rootScope, $location, $window) {
		...
		// =========== function create =================
		$scope.save = function(){
			IngredientTypesService.updateGlass($scope.data_detail, $rootScope.userLogin.token).success(function(res){
				toastr.success('Updated success!');
				setTimeout(function(){
					res.id > 0 && ($window.location.href = '#/manager-drink/ingredient-types/list');
				}, 300);
			}).error(function(err, status, res){
				console.log(err)
				toastr.error('Error!');
